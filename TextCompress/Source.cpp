...
int main(int argc, char *argv[]) {
  {
    ...
    if (argc < 4 || argc > 5 || strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "-H") == 0) {
      std::cout << "MNRC-LC" << std::endl
                << "Minimal Non-Redundant Coding for Lossless Compression" << std::endl
                << "by Antonio Esposito and Vincenzo Chianese" << std::endl
                << "Esegue la compressione/decompressione lossless del file indicato mediante la codifica MNRC-LC"
                << std::endl
                ...
                << "	Power: \t\t specifica la potenza di compressione. Se omesso, verra' impostato a 1 di default"
                << std::endl;
      return 0;
    }
